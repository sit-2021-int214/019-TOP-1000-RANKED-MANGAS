...
### ❓ Question 1: ความหลากหลายของประเภทมังงะ มีผลต่อความสนใจของผู้อ่านมากขึ้นหรือไม่
Hypothesis: ความหลากหลายของประเภทมังงะมีผลต่อความสนใจของผู้อ่าน เพราะยิ่งมังงะมีหลายประเภททำให้มีหลายอารมณ์ทำให้เข้าถึงกลุ่มเป้าหมายได้หลากหลายกลุ่มมากขึ้น
``` ruby
...
สรุปได้ว่า จำนวนประเภทของมังงะที่หลากหลายไม่มีผลต่อความสนใจของผู้อ่าน ซึ่งไม่ตรงกับสมมติฐานข้างต้น
```
...
### ❓ Question 2: มังงะในยุคเก่ามีความนิยมมากกว่ามังงะในยุคใหม่หรือไม่
Hypothesis: มังงะในยุคเก่า(ช่วงปี 1980 - 2000)เป็นที่นิยมมากกว่า เพราะความคลาสสิกของเนื้อเรื่องในสมัยก่อน อีกทั้งมังงะเก่า ๆ ยังเป็นจุดเริ่มต้นของมังงะเรื่องใหม่ ๆ อีกด้วย
``` ruby
...
### ❓ Question 3: สถานะการออกอากาศของมังงะ มีผลต่อจำนวนความสนใจของผู้อ่านหรือไม่
Hypothesis: สถานะการออกอากาศมีผลต่อจำนวนความสนใจของผู้อ่าน โดยตั้งสมมติฐานไว้ว่าสถานะของมังงะที่จบแล้วนั้นมีความสนใจต่อผู้อ่านมากกว่า เพราะกลุ่มลูกค้าอาจจะชอบดูเรื่องที่จบไปแล้วจะได้ไม่ต้องรอ และไม่ต้องค้างคา
``` ruby
# หาตารางแสดงผล Title, Status, Members
StatusAndMembersManga <- Mangas %>% 
  select(Title, Status, Members) %>% 
  arrange(desc(Members))
StatusAndMembersManga

# หาค่าเฉลี่ยของมังงะที่มีสถานะ "Finished"
FinishedManga <- StatusAndMembersManga %>% 
  filter(Status == "Finished")
count(FinishedManga)
meanFinishedManga <- sum(FinishedManga$Members)/683
meanFinishedManga

# หาค่าเฉลี่ยของมังงะที่มีสถานะ "Publishing"
PublishingManga <- StatusAndMembersManga %>% 
  filter(Status == "Publishing")
count(PublishingManga)
meanPublishingManga <- sum(PublishingManga$Members)/271
meanPublishingManga
```
Result: พบว่าจากค่าเฉลี่ยความสนใจของผู้อ่านนั้น จะมีความสนใจในมังงะที่ยังออกอากาศอยู่มากกว่ามังงะที่จบไปแล้ว ซึ่งไม่ตรงกับสมมติฐานข้างต้น
```
ตารางแสดงมังงะ สถานะ และจำนวนความสนใจของผู้อ่าน
...
ค่าเฉลี่ยมังงะที่ออกอากาศจบแล้ว(Finished) = 31497.45
ค่าเฉลี่ยมังงะที่ยังออกอากาศอยู่(Publishing) = 33066.46
```


### ❓ Question 4: จำนวนตอน(Chapters)ที่มาก ส่งผลให้จำนวนความชื่นชอบของผู้อ่านลดน้อยลงหรือไม่
Hypothesis: จำนวนตอน(Chapters)ที่มาก ไม่มีผลต่อจำนวนความชื่นชอบของผู้อ่าน เพราะยิ่งมีจำนวนตอนที่เยอะไม่ได้หมายความว่าจะมีจำนวนผู้ชื่นชอบน้อยลง
``` ruby
...
```
Result: จากผลลัพธ์พบว่าค่าเฉลี่ยความชื่นชอบของผู้อ่านจาก 10 อันดับแรกของมังงะที่มีจำนวนตอนมากที่สุดมีค่าเฉลี่ยมากกว่าค่าเฉลี่ยรวมทั้งหมด ทำให้สรุปได้ว่ามังงะที่มีจำนวนตอนเยอะไม่ส่งผลให้ความชื่นชอบของผู้อ่านลดน้อยลง ซึ่งเป็นไปตามสมมติฐาน
```
...
### ❓ Question 5: มังงะที่ได้รับความนิยม 10 อันดับแรก(Rank) สถานะปัจจุบันเป็นอย่างไร หากตีพิมพ์จบไปแล้ว ทำไมจึงยังได้รับความนิยมมาจนถึงปัจจุบันอยู่ 
Hypothesis: การ์ตูนที่ได้รับความนิยม 10 อันดับแรก (Rank) สถานะปัจจุบันยังตีพิมพ์ไม่จบเป็นส่วนใหญ่ เพราะ ยังมีความน่าติดตามต่อทำให้ได้รับความนิยมมาจนถึงปัจจุบัน
``` ruby
Mangas %>% 
