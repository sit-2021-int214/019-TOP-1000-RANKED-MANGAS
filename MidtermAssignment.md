...
## Steps 🪜
0. Download dataset and Library dplyr, stringr, readr, data.table
...
3. TOP 10 มังงะที่ได้รับคะแนนความนิยม(Score) พร้อมกับเรื่องย่อ(Synopsis)
4. เปรียบเทียบความนิยมโดยอิงจากค่าเฉลี่ย(AVG)คะแนน(Score)ของมังงะเรื่องนั้น ๆ ตามประเภทว่าระหว่าง 'Action' กับ 'Fantasy' ประเภทไหนได้รับความนิยมมากกว่ากัน
5. TOP 5 มังงะยอดนิยมที่เริ่มต้นตีพิมพ์ในปี 2001
6. ค่าเฉลี่ย(AVG)คะแนน(Score)ของมังงะที่หยุดตีพิมพ์
7. TOP 5 มังงะที่มีจำนวนเล่มเยอะที่สุด(Volumns)พร้อมกับแสดงผลอันดับของเรื่องนั้น ๆ (Rank)
8. หาสัดส่วนเปอร์เซนต์ของมังงะว่าส่วนใหญ่แล้วมังงะ TOP 1000 ยังมีสถานะการออกอากาศมากหรือน้อยกว่าเรื่องที่จบไปแล้ว
9. TOP 5 มังงะที่ผู้ใช้งานเก็บเรื่องนั้นไว้ใน Reading List ของตนเอง
10. 5 มังงะที่ไม่ค่อยได้รับความนิยมจากสมาชิก MAL โดยผู้ใช้งานไม่ค่อยเอาเข้าในลิสต์ที่ชื่นชอบ(Favorites)
...
## STEP 4 Data Analysis with Descriptive Statistics 🧐
...
Mangas %>% 
  ...
  head(n = 5L)
```
Result: สามารถนำข้อมูลไปใช้ดูมังงะที่จบแล้วและมีความยอดนิยม(Popularity) สำหรับกลุ่มลูกค้าที่ต้องการอ่านเรื่องที่นิยมและจบแล้ว โดยมีผลลัพธ์ดังนี้
```
...
Mangas %>% 
  ...
  head(n = 10L)
```
Result: สามารถนำข้อมูลความนิยมและเรื่องย่อ ไปใช้ประกอบเว็บไซต์แนะนำมังงะได้ โดยมีผลลัพธ์ดังนี้
```
...
```
Result: สามารถนำข้อมูลเปรียบเทียบระหว่างประเภท 'Action' กับ 'Fantasy' มาหาความนิยมในปัจจุบันว่ากลุ่มลูกค้าสนใจประเภทไหนมากกว่ากัน จากผลลัพธ์ที่ได้พบว่ามังงะในหมวดหมู่ Action ค่อนข้างได้รับความนิยมมากกว่ามังงะในหมวดหมู่ Fantasy
```
ActionMean = 8.203588
...
Date %>% 
  ...
  head(n = 5L)
```
Result: สามารถนำข้อมูลข้อมูลไปหาความนิยมของคนในยุคนั้นได้ว่าแนวทางอนิเมะที่ดูนั้นเป็นอย่างไร โดยมีผลลัพธ์ดังนี้
...
### ❓ Question 6: ค่าเฉลี่ย(AVG)คะแนน(Score)ของมังงะที่หยุดตีพิมพ์
``` ruby
AvgMangaOnHiatus <- Mangas %>% select(Title,Score,Status) %>% filter(Mangas$Status == "On Hiatus")
AvgMangaOnHiatus
AvgMangaOnHiatus$Score %>% mean()
```
Result: สามารถนำข้อมูลค่าเฉลี่ยมาเทียบดูได้ว่าเพราะเหตุอะไรมังงะในกลุ่มนั้นจึงถูกหยุดตีพิมพ์ไป เป็นเพราะว่าได้รับความนิยมที่ไม่เยอะหรือไม่ โดยจากค่าเฉลี่ยนั้นมีค่าออกมาเป็น 8.296667 ถือว่าเป็นคะแนนที่ค่อนข้างสูง
```
  Title                   Score Status   
...
Mangas %>% 
  ...
  head(n=5L)
```
Result: สามารถนำข้อมูลมาใช้หาได้ว่าการที่มังงะมีจำนวนเล่มเยอะ ส่งผลต่ออันดับความนิยมของผู้อ่านหรือไม่ โดยจากที่เห็นอันดับของมังงะทั้ง 5 ค่อนข้างอยู่ในช่วงกลาง ๆ เป็นส่วนใหญ่ โดยมีผลลัพธ์ดังนี้
```
  Title                           Volumns  Rank
1 Major                                78   415
2 Gintama                              77    78
...
### ❓ Question 8: หาสัดส่วนเปอร์เซนต์ของมังงะว่าส่วนใหญ่แล้วมังงะ TOP 1000 ยังมีสถานะการออกอากาศมากหรือน้อยกว่าเรื่องที่จบไปแล้ว
``` ruby
Publishing <- Mangas %>% 
  select(Title) %>% 
...
Mangas %>% 
  ...
  head(n = 5L)
```
Result: สามารถนำข้อมูลมาใช้ดูความนิยมของกลุ่มลูกค้าในเว็บไซต์ MAL ในปัจจุบัน โดยมีผลลัพธ์ดังนี้
```
  Title              Memebers
...
### ❓ Question 10: 5 มังงะที่ไม่ค่อยได้รับความนิยมจากสมาชิก MAL โดยผู้ใช้งานไม่ค่อยเอาเข้าในลิสต์ที่ชื่นชอบ(Favorites)
``` ruby
Mangas %>% 
  ...
  head(n = 5L)
```
Result: สามารถนำข้อมูลมาใช้ดูมังงะที่ไม่ค่อยได้รับความนิยมจากกลุ่มลูกค้าในเว็บไซต์ MAL ในปัจจุบัน โดยมีผลลัพธ์ดังนี้
```
  Title                          Favorites
